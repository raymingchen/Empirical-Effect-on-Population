...
{
intialOrderingMatrix=matrix(0,26,T);
iNTERPartIndex=INTERPartIndex(L,T);
  for(t in 1:T)
  {
...
AAFab[t]=sum(unlist(as.matrix(AFab[t,,])),na.rm=TRUE)/26;
}
...
seed_26_8_mt=matrix(nrow=26,ncol=8);   
for(t in 1:8)
{
seedVec=vector();
for(k in 2:(L+1))
...
seed_26_8_mt[,t]=seedVec;        ##  seed content matrix
}
...
mean_L_L_mx[L+1,L+1]=sum(unlist(part_L_L_ls),na.rm=T)/26;
return(mean_L_L_mx);
}
...
for(t in 1:8)
{
    for(i in 1:factorial(L))
    {
        for(j in 1:factorial(L))
        {
        AAE_8_facL_facL_ls[[t,i,j]]=sum(unlist(abs_8_facL_facL_ls[[t,i,j]]),na.rm=T)/26;
        }
    }
}
...
for(t in 1:8)
{
    for(i in 1:factorial(L))
    {
          ...
          AAVfe_8_facL_facL_ls[[t,i,j]]=sum/26;
          }
     }
}
